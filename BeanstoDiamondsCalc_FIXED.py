...
class BeansToDiamondsCalculator:
    def __init__(self):
        self.conversion_tiers = [
            ...
            ConversionTier(4000, 10999, 0.2768, 27.66, 3045),
    ...
    def calculate_diamonds(self, beans: int) -> Optional[Dict]:
        ...
        breakdown, total_diamonds = self.optimize_beans(beans)

        # Find which tier the first breakdown segment belongs to
        main_tier = breakdown[0]['tier'] if breakdown else None
        diamonds_per_bean = total_diamonds / beans if beans else 0
        efficiency = diamonds_per_bean * 100

        return {
            'diamonds': total_diamonds,
            'remainder': 0,
            'efficiency': round(efficiency, 2),
            'diamonds_per_bean': round(diamonds_per_bean, 4),
            'tier': main_tier
        }
